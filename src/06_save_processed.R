...
combined_taxa_long <- bind_rows(
    taxa_long_list$phylum |> mutate(level = "phylum", taxa = Phylum) |> select(-Phylum),
    taxa_long_list$family |> mutate(level = "family", taxa = Family) |> select(-Family),
    taxa_long_list$genus |> mutate(level = "genus", taxa = Genus) |> select(-Genus),
    taxa_long_list$species |> mutate(level = "species", taxa = Species) |> select(-Species)
)
