...
def main():
    ...
    participant_char.columns = (
        participant_char.columns.str.replace(
            r"^\d+\.\s*", "", regex=True
        )  # Removes "3. " but ignores "19 years"
        ...
        .str.replace(
            r"[_\.]+$", "", regex=True
        )  # Removes trailing underscores or periods
    )

    # participant ID entry cleaning
    participant_char["participant_id"] = (
        participant_char["participant_id"]
        .str.upper()
    ...
    # age entry cleaning
    participant_char["age"] = pd.to_numeric(
        participant_char["age"], errors="coerce"
    )  # force to numeric
    participant_char.loc[(participant_char["age"] < 0), "age"] = (
        np.nan
    )  # mask unrealistic ages with NANs
    ...
    participant_char["age"] = participant_char["age"].astype(
        int
    )  # cast to int type for cleaner display
    ...
    participant_char["gender"] = participant_char["gender"].fillna(
        "unknown"
    ...
    participant_char["gender_code"] = (
        participant_char["gender"].map(mapping).astype("Int64")
    )
    ...
    participant_char["eth_grouped"] = (
        participant_char["ethnicity"]
        .map(similar_map)
        .fillna(participant_char["ethnicity"])
    )

    # clean country of origin entries, use the country converter package to map to ISO3 codes
    cc = coco.CountryConverter()

    participant_char["country_of_origin"] = (
        participant_char["country_of_origin"].str.strip().str.lower()
    )
    ...
    impossible_timeline_mask = (
        participant_char["years_living_in_canada"] > participant_char["age"]
    )

    violation_count = impossible_timeline_mask.sum()
    if violation_count > 0:
        print(
            f"WARNING: Found {violation_count} rows where years in Canada > age. Setting to NaN."
        )

    participant_char.loc[impossible_timeline_mask, "years_living_in_canada"] = np.nan

    # weight(lbs) and height (cm) entry cleaning
    participant_char["weight_(lbs)"] = pd.to_numeric(
        participant_char["weight_(lbs)"], errors="coerce"
    )
    participant_char["height_(cm)"] = pd.to_numeric(
        participant_char["height_(cm)"], errors="coerce"
    )
    ...
    impossible_bmi_mask = (participant_char["bmi_1"] < 10) | (
        participant_char["bmi_1"] > 100
    )
    ...
    participant_char["exercise_history"] = (
        participant_char["exercise_history"].str.strip().str.lower()
    )
    ...
    participant_char["exercise_history"] = participant_char["exercise_history"].map(
        exercise_map
    )
    ...
    participant_char = participant_char.rename(
        columns={"comorbidities_(leave_blank_if_none)": "comorbidities"}
    )

    participant_char["comorbidities"] = (
        participant_char["comorbidities"]
        .str.lower()
        .str.strip()  # Remove exterior whitespace
        .fillna("none")
    ...
    participant_char["family_history_of_ibd"] = (
        participant_char["family_history_of_ibd"].str.strip().str.lower()
    )

    # convert to a binary field
    binary_map = {"yes": 1, "no": 0}

    participant_char["family_history_of_ibd"] = participant_char[
    ...
    participant_char = participant_char.rename(
        columns={"smoklng_status": "smoking_status"}
    )
    ...
    participant_char["smoking_status"] = participant_char["smoking_status"].astype(
        smoke_cat_type
    )

    # alcohol intake entry cleaning
    participant_char["alcohol_intake"] = (
        participant_char["alcohol_intake"].str.strip().str.lower()
    )
    ...
    participant_char["alcohol_intake"] = participant_char["alcohol_intake"].map(
        alcohol_ordinal_map
    )
    ...
    participant_char["recreational_drug_use"] = (
        participant_char["recreational_drug_use"].str.strip().str.lower()
    )
    participant_char["recreational_drug_use"] = (
        participant_char["recreational_drug_use"].map(binary_map).astype("Int64")
    )
    ...
    participant_char["antbiotics_last_2months"] = (
        participant_char["antbiotics_last_2months"].str.strip().str.lower()
    )
    participant_char["antbiotics_last_2months"] = (
        participant_char["antbiotics_last_2months"].map(binary_map).astype("Int64")
    )
    ...
    participant_char["supp_prebiotics"] = (
        participant_char["supp_prebiotics"].str.strip().str.lower()
    )
    participant_char["supp_prebiotics"] = (
        participant_char["supp_prebiotics"].map(binary_map).astype("Int64")
    )
    ...
    participant_char["probiotics"] = (
        participant_char["probiotics"].str.strip().str.lower()
    )
    participant_char["probiotics"] = (
        participant_char["probiotics"].map(binary_map).astype("Int64")
    )
    ...
    participant_char["postbiotics"] = (
        participant_char["postbiotics"].str.strip().str.lower()
    )
    participant_char["postbiotics"] = (
        participant_char["postbiotics"].map(binary_map).astype("Int64")
    )
    ...
    participant_char.loc[
        participant_char["supp_prebiotics"] == 0, "brand_prebiotics"
    ] = "none"
    participant_char.loc[participant_char["probiotics"] == 0, "brand_probiotics"] = (
        "none"
    )
    participant_char.loc[participant_char["postbiotics"] == 0, "brand_postbiotics"] = (
        "none"
    )

    # keep text entries consistently formatted
    participant_char["brand_prebiotics"] = (
        participant_char["brand_prebiotics"].str.lower().str.strip()
    )
    participant_char["brand_probiotics"] = (
        participant_char["brand_probiotics"].str.lower().str.strip()
    )
    participant_char["brand_postbiotics"] = (
        participant_char["brand_postbiotics"].str.lower().str.strip()
    )
    ...
    participant_char["general_well-being"] = participant_char[
        "general_well-being"
    ].replace(
        {
            "Poor = 2": "Poor = 0",
    ...
    # weight(lbs) and height (cm) entry cleaning
    participant_char["weight_(lbs)_.1"] = pd.to_numeric(
        participant_char["weight_(lbs)_.1"], errors="coerce"
    )
    participant_char["height_(cm)_.1"] = pd.to_numeric(
        participant_char["height_(cm)_.1"], errors="coerce"
    )

    participant_char.loc[participant_char["height_(cm)_.1"] <= 0, "height_(cm)_.1"] = (
        np.nan
    )
    ...
    # filter out impossible bmi entries
    impossible_bmi_mask = (participant_char["bmi_2"] < 10) | (
        participant_char["bmi_2"] > 100
    )
